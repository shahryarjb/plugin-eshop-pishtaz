...
class eshop_postcost_pishtaz   extends eshop_shipping
{
    ...
    function getQuote($addressData, $params)
    {
        ...
        if($sourceZone == 0 )
		{
			echo '<p></p>'.'<font color="red">'.JText::_('PLG_ESHOP_POSTCOST_PISHTAZ_ERROR2').'</font>'.'<p></p>';
		}
        //=================== get geo of dest
        if ($curentUser == 9) {
            // not registered
        ...
        //============= define variable
        //---------------------------------------------------// beyne shahri
        $weight250_BS   = $params->get('pishtaz_250_BS');
        $weight500_BS   = $params->get('pishtaz_500_BS');
        $weight1000_BS  = $params->get('pishtaz_1000_BS');
        ...
        $query->clear();
        $query->select('*')->from('#__eshop_shippings')->where('name = "eshop_postcost_pishtaz"');
        $db->setQuery($query);
        $row  = $db->loadObject();
		...
		{
			echo '<p></p>'.'<b><font color="red">'.JText::_('PLG_ESHOP_POSTCOST_PISHTAZ_ERROR').'</font></b>';
		}
